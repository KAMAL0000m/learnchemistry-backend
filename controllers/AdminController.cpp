...
namespace learnChemistry::controllers {
            ...
            learnChemistry::context::RequestContext& ctx) const
    {
        ...
        if (!isAdmin(ctx)) {
            return HttpResponse::json(http::status::forbidden, { {"error","Admin only"} });
        }
        ...
        if (title.empty()) {
            return HttpResponse::json(http::status::bad_request, { {"error","title required"} });
        }
